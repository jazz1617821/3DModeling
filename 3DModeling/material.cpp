...
mtl_t* newMaterial(void)
{
	mtl_t *mtl;
	
	mtl = (mtl_t*)calloc(1, sizeof(mtl_t));
	if (mtl == NULL) {
		assert("NULL...\n");
	}
	setMatAmbient(mtl, .1, .18725, .1745, 1.0);
	setMatDiffuse(mtl, .396, .74151, .69102, 1.0);
	setMatSpecular(mtl, .297254, .30829, .306678, 1.0);
	setMatShininess(mtl, 0.1 * 128.0);
	mtl->isEnable = true;

	return mtl;
}

mtl_t* newDefaultMTL(int type)
{
	mtl_t* mtl;
...
void deleteMaterial(mtl_t** material)
{
	free((*material));
	*material = NULL;
}

void setMatAmbient(mtl_t* material, float r, float g, float b, float a)
{
...
void setMatSpecular(mtl_t* material, float r, float g, float b, float a)
{
...
void setMatDiffuse(mtl_t* material, float r, float g, float b, float a)
{
...
void setMatShininess(mtl_t* material, float shininess)
{
...
mtl_t* readMaterial(const char* fileName)
{
	mtl_t* mtl;
	FILE* fp;
