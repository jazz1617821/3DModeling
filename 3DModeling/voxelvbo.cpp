...
vbo_t* createVoxelVBO(vobj_t* vo)
{
	...
	vdata_t* vd = vo->dataItem->voxlData;
	voxel_t* vox = vo->dataItem->voxlData->rawData;
	vbo_t* vbo;
	vbo_t* boxVBO;
	vbo_t* boxWFVBO;

	origin[X] = -(vo->bbox->minBound[X]/2);
	origin[Y] = -(vo->bbox->minBound[Y]/2);
	origin[Z] = -(vo->bbox->minBound[Z]/2);
